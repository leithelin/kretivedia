$(document).ready(function () {
    ...
    }, "Jméno musí obsahovat minimálně 2 znaky a nesmí obsahovat zvláštní znaky ani mezery");

    $.validator.addMethod("lastusernameRegex", function (value, element) {
        return this.optional(element) || regex_first_last_name.test(value);
    }, "Příjmení musí obsahovat minimálně 2 znaky a nesmí obsahovat zvláštní znaky ani mezery");

    $.validator.addMethod("passwordRegex", function (value, element) {
        return this.optional(element) || /[a-z]/.test(value) && /[0-9]/.test(value) && /[A-Z]/.test(value) && /^[0-9A-Za-z]+$/.test(value);
    }, 'Heslo musí obsahovat 8-12 znaků, včetně písmen (A-Z) a čísel (0-9). Nesmí obsahovat zvláštní znaky (^@()#*+/\"?!=.{}~`&) ani mezery');

    $.validator.addMethod("phoneRegex", function (value, element) {
        return this.optional(element) || /^(\d[- ]?){7,11}$/.test(value);
    }, "Telefonní číslo musí obsahovat 7-11 znaků bez zvláštních znaků");
    ...
    $(function () {
        ...
        form.validate({
            ...
            messages: {
                first_name: {
                    required: "Je potřeba vyplnit jméno",
                    minlength: "Jméno musí obsahovat alespoň 2 znaky",
                    maxlength: "Jméno může obsahovat maximálně 25 znaků",
                },
  
                last_name: {
                    required: "Je potřeba vyplnit příjmení",
                    minlength: "Příjmení musí obsahovat alespoň 2 znaky",
                    maxlength: "Příjmení může obsahovat maximálně 25 znaků",
                },
                password: {
                    required: "Je potřeba vyplnit heslo",
                    minlength: "Heslo musí obsahovat alespoň 8 znaků",
                    maxlength: "Heslo nesmí být delší než 12 znaků",
                },
                email: {
                    required: "Je potřeba vyplnit email",
                    email: "Emailová adresa je neplatná",
                },
                phone: {
                    required: "Je potřeba vyplnit telefonní číslo",
                },
